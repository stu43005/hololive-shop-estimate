...
- 不改動既有 variant prefix `セット` 過濾邏輯（`_strip_prefix` 相關）。
...
peer 集合定義為「同一次 `decompose_items` 呼叫產出的其他 `ProductItem`」（即同商品
拆解後、去重後的其他 item），peer 價格取 `price_jpy`。
...
- `from_yaml`/`load_config` 解析 `bundle_set` 區塊：缺少區塊時建立空 `keywords`/
  `keep_keywords`、`price_ratio` 用預設 5.0 的 `BundleSetPolicy`（= 不啟用任何
  關鍵字排除，但 (B) 仍可能在無 keep_keywords 時對極高 ratio 的 set 生效；
  本專案在 yaml 顯式提供完整三項，預設空清單只影響未設定此區塊的其他環境）。
...
- `decompose_items` 簽名新增 keyword-only 參數 `bundle_set: BundleSetPolicy`
  （放在既有 `talents` 之後）。為避免 `items.py` 依賴 `config_schema` 造成循環
  import，改以**結構化參數**傳入：`decompose_items` 接收三個基本型別參數
  `bundle_keywords: frozenset[str]`、`bundle_price_ratio: float`、
  `bundle_keep_keywords: frozenset[str]`，由呼叫端從 `BundleSetPolicy` 取出傳入。
  （`items.py` 既有風格即以 `talents: frozenset[str]` 等基本型別為介面，
  延續一致。）
...
`bundle_set`（或其拆出的三個基本型別參數）沿用 `talents` 既有路徑，以 keyword
參數傳遞：

- `crawler/cycle.py`：`async_process_queue(..., talents=config.talents, ...)` 旁
  新增 `bundle_set=config.bundle_set`。
- `crawler/async_pipeline.py`：`async_process_queue` 簽名新增
  `bundle_set: BundleSetPolicy`，並在呼叫 `sync_products(...)` 時下傳。
- `sync/engine.py`：`sync_products` 簽名新增 `bundle_set: BundleSetPolicy`，
  在呼叫 `decompose_items(snapshot, talents=talents, ...)` 時，把
  `bundle_set.keywords`、`bundle_set.price_ratio`、`bundle_set.keep_keywords`
  拆開傳給 `decompose_items` 的三個基本型別參數。
- `engine.py` 既有 log 行（約 line 115-116）可加上 bundle 計數的彙整（非必要，
  但 plan 會把 `excluded_bundle` 納入 `_aggregate_lines` 或 counters 以利觀測）。
...
```
AppConfig.from_yaml(stores_config.yaml)
  → AppConfig.bundle_set: BundleSetPolicy
  → cycle.async_process_queue(bundle_set=...)
  → async_pipeline.async_process_queue(bundle_set=...)
  → engine.sync_products(bundle_set=...)
  → decompose_items(snapshot, talents=..., bundle_keywords=..., bundle_price_ratio=..., bundle_keep_keywords=...)
