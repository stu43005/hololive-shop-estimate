...
- 不改變任何業務行為、回傳值語義、控制流或估價結果；只新增 logging 與兩個內部函式的**回傳型別**（純資訊承載，數值不變）。
- 不引入結構化（JSON）log、不加結構化欄位（`store=..` `pid=..`）——純文字樹。
- 不改 `classify_query`（bot 查詢端，不在 crawl 樹內）。
...
- embedding／typing 失敗使整個 product 進入既有 `except`（[engine.py:110](../../../estimator_king/sync/engine.py)）：維持現行 `logger.exception("Sync failed for %s")`，**該 product 不輸出樹**（已知取捨：失敗 product 看 exception log + traceback，而非樹）。
...
格式規則：
- 根行：`product {store_id}:{product_id} "{title}" ({created|updated}): {N} items, {E} excluded (SET×{s}, ¥0×{z})`。`E==0` 時省略 `, 0 excluded (...)`，只到 `{N} items`。
- 每 item 兩行：第一行 `├─`/`└─`（最後一筆用 `└─`）`item "{item_name}" ×{合併 variant 數}`，且 `talents` 數 >0 時接 ` talents={n}`；第二行縮排接 `detail=… typing=… embed=…`（最後一筆 item 的續行用空白縮排，非 item 的續行用 `│` 連接，如範例）。
...
- **typing**：`{item_type}({source})`，`source`：
  - `vocab` — 第一層受控詞彙**唯一命中**（零 LLM）。
  - `cache` — 第二層命中快取。
  - `llm` — 第二層新呼叫 LLM（含多重命中、零命中、以及例外 fallback `その他`——例外仍視為 `llm` 路徑）。
...
僅改兩個內部函式的回傳型別與一個內部 helper；數值行為不變。
...
- `classify_item(...) -> TypeDecision`：第一層唯一命中 → `TypeDecision(hit, "vocab")`；否則用 `_llm_classify` 的 `(item_type, source)` 組 `TypeDecision`。
- `classify_query(...)` **不變**（仍回 `list[str]`；其內部若呼叫 `_llm_classify` 需取 tuple 的第 0 元素 `item_type`）。
- 連動：`test_typing.py` 既有對 `classify_item` 回傳 `str` 的斷言改為 `.item_type`；新增對 `.source` 三態（vocab/cache/llm）的斷言。
...
- 連動：`test_items.py` 既有 `decompose_items(...)` 視為 list 之處改為 `.items`（約 10 處，機械式）。
...
- `_rebuild_product_items` 改為**收集**每 item 的決策列（`detail_hit`、`TypeDecision`、`embed_status ∈ {"indexed","skipped(unchanged)"}`）並回傳一個 `RebuildReport`（`item_rows: list[...]`, `excluded_set`, `excluded_zero`）供 `sync_products` 組樹。`classify_item` 改用 `.item_type` 寫 `_format_item_document`／`_item_hash`，`.source` 進報告。embed skip 分支記 `"skipped(unchanged)"`，否則 upsert 後記 `"indexed"`。
- `sync_products`：
