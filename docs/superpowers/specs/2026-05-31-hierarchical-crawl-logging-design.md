...
僅改三個內部函式的回傳型別（`classify_item`、`decompose_items`、`_rebuild_product_items`）並新增純格式化 helper `_format_product_tree`；數值行為不變。另沿鏈路新增 `log_item_trees` flag（§7.0）區分入口。
...
- 既有 crawl-path 測試（**全部五個**：`test_async_pipeline.py`、`test_async_pipeline_logging.py`、`test_integration_async_pipeline.py`、`test_crawl_cycle.py`、`test_scheduler.py`）：對 `async_process_queue`／`run_crawl_cycle` 的呼叫**不傳** `log_item_trees`（沿用預設 `False`）→ 無樹、行為不變、斷言**不需改動**。實作後須回跑這五個檔確認預設值不破壞既有斷言（含 `test_async_pipeline_logging.py` 的 store 級 `queue`／heartbeat／`done` 斷言不被樹污染）。
...
6. 既有 store 級 log（queue/heartbeat/done）與 crawl stdout JSON counters 不變。
7. 工具鏈全綠（basedpyright 0 error、ruff、pytest）。
