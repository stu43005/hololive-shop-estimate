...
- 起始值 `general=60 / premium=70` 取自上方 sweep 的兩組最佳點，上線前再校準（見 §5）。

### §2 Floor 演算法

對每筆估價（query 字串 + 它的同類 ref 價格清單）：

1. **取數基準**：同類 refs = 送進模型的 **top_k context 中 `item_type ∈ classify_query(query)` 且 `price_jpy > 0`** 的 ref 價格清單。這些是模型實際被 grounding 的同類比價。若清單為空 → floor **no-op**。
...
- 新增 `@dataclass AnchorTier(percentile: int, keywords: list[str])`、`@dataclass AnchorFloorConfig(general_percentile: int, premium_tiers: list[AnchorTier])`。
- `AppConfig` 新增欄位 `estimator_anchor_floor: AnchorFloorConfig | None`。
- `from_yaml` 解析 `estimator.anchor_floor`：缺 → `None`；存在 → 建 `AnchorFloorConfig`（`premium_tiers` 缺省為空 list）。
- `config.validate()`（結構驗證）：若 `estimator_anchor_floor` 非 None，檢查 `general_percentile` 與每個 tier 的 `percentile` 為 0–100 整數、每個 tier 的 `keywords` 為非空的非空字串清單；違反則拋與既有結構驗證一致的錯誤。
...
- config 解析/驗證：合法區塊解析為 `AnchorFloorConfig`；缺區塊 → `None`（停用）；`percentile` 越界（<0 或 >100）→ validate 報錯；空 `keywords` → validate 報錯。
...
- **校準（上線前必做）**：`general=60 / premium=70` 為 sweep 起始點。以對齊後的 `experiment_anchor_floor.py`（讀 config 分層）確認分層 signed err，再以 `eval_estimate.py --runs ≥ 3` 取 before/after，數據記入 PR / commit。
