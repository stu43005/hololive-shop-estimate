...
Run（檢查 runtime 之外無殘留的 provider 建構；`\b` word-boundary 確保 `_VectorStore(Protocol)` 這類 Protocol 定義不被誤匹配，`--exclude` 排除 runtime.py 本身）：
```bash
grep -rnE "\b(EmbeddingProvider|ChatProvider|VectorStore)\(" estimator_king/ \
  --include="*.py" --exclude="runtime.py" \
...
（註：`estimator.py` / `sync/engine.py` / `sync/inactive.py` 內有 `class _VectorStore(Protocol):` 等型別協定定義，前綴底線使 `\bVectorStore\(` 不匹配它們，避免誤報。）
