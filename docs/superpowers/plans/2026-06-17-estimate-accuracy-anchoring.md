...
**Architecture:** 改動集中在 [estimator.py](../../../estimator_king/bot/estimator.py) 的 `SYSTEM_PROMPT`（三處外科式編修）＋一個 prompt-hash 啟動屬性附加到既有日誌；新增 `scripts/analysis/eval_estimate.py`（重現 `_estimate_chunk` retrieval、套用「本尊排除」、配對多-run 量測、fail-closed）；同步 [docs/data-pipeline.md](../../../docs/data-pipeline.md) 階段 14。不動 retrieval/rerank/chat provider/資料結構/`snap_to_tax_grid` 的行為。
...
**任務順序（重要，為了正確量測 before/after）：** Task 1（hash 日誌）→ Task 2（eval 腳本）→ **跑 baseline（舊 prompt）** → Task 3（改 `SYSTEM_PROMPT`）→ **跑 candidate（新 prompt）** → Task 4（同步文件）。Task 1/2 不依賴 prompt 文字，且 eval 自行從 `SYSTEM_PROMPT` 計算 hash，因此 baseline 能在 prompt 改動前、於已含 eval 腳本的 working tree 量到舊 prompt 的數據。
...
- **Modify** [estimator_king/bot/estimator.py](../../../estimator_king/bot/estimator.py)
  - `import hashlib`；`Estimator.__init__` 加 `self._prompt_hash`；`estimate_products` 兩條 `logger.info` 附加 `prompt=%s`（Task 1）。
  - `SYSTEM_PROMPT`：`<premium_adjustment>` → `<anchoring>`；新增 `<set_and_count>`；改寫 `<range_and_confidence>`（Task 3）。
- **Modify** [tests/test_estimator_logging.py](../../../tests/test_estimator_logging.py)：新增 `prompt=` 斷言（Task 1）。
- **Create** [scripts/analysis/eval_estimate.py](../../../scripts/analysis/eval_estimate.py)：eval 工具（Task 2）。
- **Modify** [docs/data-pipeline.md](../../../docs/data-pipeline.md) 階段 14：同步 prompt 行為（Task 4）。

---

## Task 1：`Estimator` prompt-hash 屬性 + 附加到既有日誌

**Files:**
- Modify: `estimator_king/bot/estimator.py`（import 區、`__init__`、`estimate_products`）
- Test: `tests/test_estimator_logging.py`

採 TDD：先加日誌斷言（失敗）→ 實作 → 通過。此任務不改 `SYSTEM_PROMPT` 文字。
...
Expected: FAIL on `assert sum("prompt=" in m ...) >= 2`（目前日誌沒有 `prompt=`）。
...
Expected: PASS（兩條 INFO 訊息都含 `prompt=`；既有 `estimate done`/`2 estimates` 子字串仍在）。
...
## Task 2：新增 `scripts/analysis/eval_estimate.py`

**Files:**
- Create: `scripts/analysis/eval_estimate.py`

無 unit test（與 `scripts/analysis/calibrate_*.py` 慣例一致；需 live chroma + API）。驗證 = type check + lint + import smoke。腳本重現 `_estimate_chunk` retrieval、對每筆 query 剔除「本尊」、配對多-run 量測、fail-closed。檔頭以 `# pyright: reportPrivateUsage=false` 允許刻意重用 `Estimator` 內部（與 `calibrate_*.py` 重用內部檢索同精神）。腳本**自行從 `SYSTEM_PROMPT` 計算 prompt hash**，不依賴 Task 1 的屬性，故能在 Task 3（prompt 改動）之前先跑出 baseline。
...
> 註：`load_config()` 是讀 `.env` + YAML 的 env-aware 載入器（與 `calibrate_*.py` 一致）。因為 `with_chat=True` 需要 chat 憑證，故不能用不讀 env、不驗憑證的 `AppConfig.from_yaml`。retrieval、self-exclusion（`price == official` 且 名稱相符或 `similarity = 1.0 - distance >= 0.95`）、no-estimate「任一 run ¥0」聚合、per-fixture 平均、INVALID fail-closed 與 provenance 皆已落實。
...
## ⏸ Baseline 量測（在 Task 3 之前執行）

此時 `SYSTEM_PROMPT` 仍是舊版。跑一次 eval 取得 baseline，供 Task 3 之後對比。

- [ ] **Step 1：跑 baseline eval（舊 prompt）**

Run: `set -a; source .env; set +a; PYTHONPATH=. .venv/bin/python scripts/analysis/eval_estimate.py --runs 3`
Expected: 完整輸出 PER-FIXTURE 表、SUMMARY（MAPE / range 覆蓋率 / no-estimate 清單）、PROVENANCE（記下 `prompt_hash` 與 `git_commit`）。把整段 stdout 存檔（如 `/tmp/eval_baseline.txt`）。若印出 `INVALID run` → 排除 API/DB 問題後重跑，不可拿 INVALID 當 baseline。

---

## Task 3：改寫 `SYSTEM_PROMPT` 三處（#A/#B/#C）

**Files:**
- Modify: `estimator_king/bot/estimator.py:45-66`

無 unit test（prompt 文字無測試斷言；由 lint/type + candidate eval 驗證）。改動只替換字串常數內容。
...
Run: `.venv/bin/python -m pytest tests/test_estimator.py tests/test_estimator_logging.py -v -o addopts=""`
Expected: PASS（snap/retrieval/rerank/reconcile/logging 斷言不觸及 prompt 文字內容）。
...
## ⏸ Candidate 量測（在 Task 3 之後執行）

- [ ] **Step 1：跑 candidate eval（新 prompt，相同 `--runs`）**

Run: `set -a; source .env; set +a; PYTHONPATH=. .venv/bin/python scripts/analysis/eval_estimate.py --runs 3`
Expected: 完整輸出；`prompt_hash` 應與 baseline 不同（確認量到新 prompt）。把整段 stdout 存檔（如 `/tmp/eval_candidate.txt`）。

- [ ] **Step 2：依驗收準則判定**

對比 baseline 與 candidate，須**同時**滿足：MAPE 不變差（≤ baseline）、range 覆蓋率不變差（≥ baseline）、candidate 的 no-estimate 集合 ⊆ baseline 的 no-estimate 集合、至少一筆目標 fixture（ポーチ／ボイス1種／ピンバッジ2個セット 等先前低估筆）的 per-fixture 平均誤差實質下降、兩份 run 皆為 VALID（非 INVALID）。
任一條不滿足 → 回 Task 3 收斂措辭或放棄該規則（spec 已許可）；把 before/after 兩份 stdout（含 provenance）記錄於 PR / commit 訊息。
...
**Files:**
- Modify: `docs/data-pipeline.md:837-844`（階段 14 step 1 的 prompt 行為描述）與 `:869-873`（設計理由）

無 code、無測試。CLAUDE.md 強制：pipeline 行為改動須同 change 更新本文件。文中 confidence-high 修飾詞清單須與 Task 3 的 `SYSTEM_PROMPT`（collaboration/brand/series name, size, material, set count）一字對應。
