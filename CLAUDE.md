...
> **Full data-flow reference — [docs/data-pipeline.md](docs/data-pipeline.md)**. This is the authoritative end-to-end walkthrough of the whole pipeline: the **write path** (`stores_config.yaml` → sitemap enumeration → enqueue/budget → fetch → extract → content-hash change detection → decompose into items → item-type classification → embed → ChromaDB/SQLite upsert → inactive sweep, stages 0–10) and the **query path** (`/estimate` → embed query → type-aware retrieval → recency+diversity rerank → chat estimate → reconcile, stages 11–14). For each step it records the exact limit/filter/condition + format, the controlling config key, the corresponding function (`file:line`), and the design rationale ("why").
>
> **Keep it in sync — MANDATORY.** Any change to the data-pipeline flow **must update `docs/data-pipeline.md` in the same change** so it never goes stale. This includes: adding/removing/reordering a pipeline stage; changing any filter, limit, threshold, condition, or its config key; renaming a pipeline function; altering the vector ID / embedding-document / metadata format; or changing retrieval/rerank/estimate behavior. Update the affected stage's mechanics, its config table, the function references, and any "設計理由" notes that no longer hold. Treat the doc as part of the pipeline's definition of done, not optional follow-up.
